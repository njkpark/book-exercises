...
names <- read.csv("~/Desktop/book-exercises/chapter-10-exercises/exercise-5/data/female_names.csv", stringsAsFactors = FALSE)

# Create a data frame `names_2013` that contains only the rows for the year 2013
names_2013 <- names[names$year == 2013, ]

# What was the most popular female name in 2013?
most_popular <- names_2013[names_2013$prop == max(names_2013$prop), "name"]
...
most_popular_in_a_year <- function(year) {
  ...
  popular_name <- year_names[year_names$prop == max(year_names$prop), "name"]
  return(popular_name)
}
...
number_in_million <- function(name, year) {
  same_name <- names[names$year == year & names$name == name, "prop"]
  round(same_name * 1000000, 1)
}
