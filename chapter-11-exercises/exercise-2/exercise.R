...
hwy_1997 <- arrange(cars_1997, hwy)
# Mutate the 1997 cars data frame to add a column `average` that has the average
# gas milage (between city and highway mpg) for each car
mutate(cars_1997, average = ((cars_1997$hwy + cars_1997$cty) / 2))
...
worst_hwy <- filter(wheel_2, hwy == min(hwy))
worst_id <- select(worst_hwy, id)
...
most_hwy <- function(year_choice, make_choice) {
  year_make <- filter(vehicles, year == year_choice & vehicles, make == make_choice)
  model <- filter(year_make, hwy == max(hwy), "model")
}
